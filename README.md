...
`gui/fonts/metron-bold14`. Имена шрифтов больших кеглей я подтвердить не
смог, а выдуманное имя даёт невидимый текст — поэтому крупная типографика
(TITLE 96, таймер 92, процент 40) сейчас идёт 22-м кеглем. Логотип это не
затрагивает: он текстура. Если в вашем билде есть шрифт крупнее — это
замена одной строки `font` у нужных виджетов в `tools/gen_layouts.py`.

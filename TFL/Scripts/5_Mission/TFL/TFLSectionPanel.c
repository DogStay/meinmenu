...
class TFL_SectionRow
{
    ...
    void TFL_SectionRow(string name, string map, string players, string mode, string ping, string description = "")
    {
        Name        = name;
        Map         = map;
        Players     = players;
...
class TFL_SectionPanel
{
    static const string LAYOUT   = "TFL/GUI/Layouts/section_panel.layout";
    static const int    MAX_ROWS = 6;

    protected Widget            m_Root;
    protected TextWidget        m_Desc;
    protected ref TFL_MenuButton m_Action;

    protected ref array<Widget>          m_Rows;
    protected ref array<ref TFL_SectionRow> m_Data;
    protected int m_Selected;

    void TFL_SectionPanel(Widget parent)
    {
        m_Root = GetGame().GetWorkspace().CreateWidgets(LAYOUT, parent);

        if (!m_Root)
        {
            ErrorEx("[TFL] section_panel.layout не загрузился");
            return;
        }

        m_Desc     = TextWidget.Cast(m_Root.FindAnyWidget("tfl_section_desc"));
        m_Rows     = new array<Widget>;
        m_Selected = -1;

        for (int i = 0; i < MAX_ROWS; i++)
        {
            Widget row = m_Root.FindAnyWidget(string.Format("tfl_row_%1", i));
        ...
        ButtonWidget action = ButtonWidget.Cast(m_Root.FindAnyWidget("tfl_section_action"));

        if (action)
            m_Action = new TFL_MenuButton(action, TFL_BtnStyle.PRIMARY);
    }

    void ~TFL_SectionPanel()
    {
        Destroy();
    }
    ...
    void SetRows(array<ref TFL_SectionRow> rows)
    {
        m_Data     = rows;
        m_Selected = -1;
        ...
        for (int i = 0; i < m_Rows.Count(); i++)
        {
            ...
            TFL_SectionRow data = rows.Get(i);
    ...
    protected void ApplyRowStyle(Widget row, bool selected)
    {
        ...
        if (fill)
            fill.SetColor(selected ? TFL_Theme.WithAlpha(TFL_Theme.OLIVE_DARK, 0.10) : TFL_Theme.PANEL);

        if (mark)
            mark.SetColor(selected ? TFL_Theme.OLIVE : TFL_Theme.TRANSPARENT);
    }
}

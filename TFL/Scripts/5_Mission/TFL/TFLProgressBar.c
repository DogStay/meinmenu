...
class TFL_ProgressBar
{
    ...
    void TFL_ProgressBar(Widget root)
    {
        if (!root)
            return;

        m_Track   = root.FindAnyWidget("tfl_progress_track");
        m_Percent = TextWidget.Cast(root.FindAnyWidget("tfl_progress_percent"));

        if (!m_Track)
            return;

        m_Bg   = m_Track.FindAnyWidget("tfl_progress_bg");
        m_Fill = m_Track.FindAnyWidget("tfl_progress_fill");
        m_Caps = m_Track.FindAnyWidget("tfl_progress_caps");

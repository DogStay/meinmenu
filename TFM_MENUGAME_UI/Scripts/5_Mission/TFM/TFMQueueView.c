...
class TFM_QueueView
{
    static const string LAYOUT      = "TFM_MENUGAME_UI/GUI/Layouts/server_queue.layout";
    static const float  DOT_PERIOD  = 1.2;
    static const int    DOT_COUNT   = 3;

    protected Widget            m_Root;
    protected ref TFM_HintCard  m_Hint;
    protected ref TFM_MenuButton m_Cancel;

    protected ref TFM_Digits    m_Position;   //!< 88px — текстурные глифы
    ...
    void TFM_QueueView()
    {
        ...
        if (!m_Root)
        {
            ErrorEx("[TFM] server_queue.layout не загрузился");
            return;
        }

        m_Position = new TFM_Digits(m_Root, "tfm_queue_digits", 3);
        m_Total    = TextWidget.Cast(m_Root.FindAnyWidget("tfm_queue_total"));

        ButtonWidget cancel = ButtonWidget.Cast(m_Root.FindAnyWidget("tfm_queue_cancel"));

        if (cancel)
            m_Cancel = new TFM_MenuButton(cancel, TFM_BtnStyle.DANGER);

        m_Dots = new array<Widget>;

        for (int i = 0; i < DOT_COUNT; i++)
        {
            Widget dot = m_Root.FindAnyWidget(string.Format("tfm_queue_dot_%1", i));
        ...
        Widget holder = m_Root.FindAnyWidget("tfm_queue_hint_holder");

        if (holder)
            m_Hint = new TFM_HintCard(holder, TFM_HintCard.L_FULL, false);
    }
    ...
    protected void UpdateDots(float timeslice)
    {
        ...
        for (int i = 0; i < m_Dots.Count(); i++)
        {
            ...
            m_Dots.Get(i).SetColor(TFM_Theme.WithAlpha(TFM_Theme.OLIVE, alpha));
        }
    }
}

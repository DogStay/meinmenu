...
class TFM_TimerView
{
    static const string LAYOUT = "TFM_MENUGAME_UI/GUI/Layouts/timer_panel.layout";

    protected Widget            m_Root;
    protected Widget            m_Panel;
    protected Widget            m_Mark;
    protected TextWidget        m_Title;
    protected ref TFM_Digits    m_Value;   //!< 92px — текстурные глифы
    ...
    protected ref TFM_MenuButton m_Respawn;

    protected TFM_TimerMode m_Mode;
    protected float m_Total;
    protected float m_Left;
    protected float m_PulseTime;
    protected float m_TrackW, m_TrackH;

    void TFM_TimerView(TFM_TimerMode mode)
    {
        m_Mode = mode;
        ...
        if (!m_Root)
        {
            ErrorEx("[TFM] timer_panel.layout не загрузился");
            return;
        }
        ...
        Widget panel = m_Root.FindAnyWidget("tfm_timer_panel");

        if (panel)
            m_Panel = panel.FindAnyWidget("border");
        m_Mark      = m_Root.FindAnyWidget("tfm_timer_mark");
        m_Title     = TextWidget.Cast(m_Root.FindAnyWidget("tfm_timer_title"));
        m_Value     = new TFM_Digits(m_Root, "tfm_timer_digits", 4);
        m_Subtitle  = TextWidget.Cast(m_Root.FindAnyWidget("tfm_timer_subtitle"));
        m_Character = TextWidget.Cast(m_Root.FindAnyWidget("tfm_timer_character"));
        m_Hint      = TextWidget.Cast(m_Root.FindAnyWidget("tfm_timer_hint"));
        m_Track     = m_Root.FindAnyWidget("tfm_timer_track");

        if (m_Track)
        {
            m_Fill = m_Track.FindAnyWidget("tfm_timer_fill");

            if (m_Fill)
                m_Fill.GetSize(m_TrackW, m_TrackH);
        }

        ButtonWidget respawn = ButtonWidget.Cast(m_Root.FindAnyWidget("tfm_timer_respawn"));

        if (respawn)
            m_Respawn = new TFM_MenuButton(respawn, TFM_BtnStyle.DANGER);
    ...
    void SetTime(float seconds, float total = -1)
    {
        ...
        if (m_Mode == TFM_TimerMode.RESPAWN && m_Respawn)
        {
            bool ready = m_Left <= 0;
    ...
    {
        bool respawn = m_Mode == TFM_TimerMode.RESPAWN;

        if (m_Mark)
            m_Mark.SetColor(respawn ? TFM_Theme.RED : TFM_Theme.OLIVE);

        if (m_Panel)
            m_Panel.SetColor(respawn ? TFM_Theme.BORDER_DANGER : TFM_Theme.BORDER);

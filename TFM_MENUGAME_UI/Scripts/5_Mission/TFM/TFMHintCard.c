...
class TFM_HintCard
{
    static const string L_FULL    = "TFM_MENUGAME_UI/GUI/Layouts/hint_card.layout";
    static const string L_COMPACT = "TFM_MENUGAME_UI/GUI/Layouts/hint_card_compact.layout";
    static const string L_WIDE    = "TFM_MENUGAME_UI/GUI/Layouts/hint_card_wide.layout";
    ...
    protected ref TFM_MenuButton    m_Prev;
    protected ref TFM_MenuButton    m_Next;

    protected ref array<ref TFM_Hint> m_Hints;
    ...
    void TFM_HintCard(Widget parent, string layout, bool showArrows = true)
    {
        m_Hints = TFM_Hints.Get();
        m_Index = 0;
        m_Fade  = 1;
        ...
        if (!m_Root)
        {
            ErrorEx("[TFM] hint card layout не загрузился: " + layout);
            return;
        }

        m_Image    = ImageWidget.Cast(m_Root.FindAnyWidget("tfm_hint_image"));
        m_ImageDim = m_Root.FindAnyWidget("tfm_hint_image_dim");
        m_Category = TextWidget.Cast(m_Root.FindAnyWidget("tfm_hint_category"));
        m_Title    = TextWidget.Cast(m_Root.FindAnyWidget("tfm_hint_title"));
        m_Desc     = MultilineTextWidget.Cast(m_Root.FindAnyWidget("tfm_hint_desc"));
        m_Counter  = TextWidget.Cast(m_Root.FindAnyWidget("tfm_hint_counter"));

        ButtonWidget prev = ButtonWidget.Cast(m_Root.FindAnyWidget("tfm_hint_prev"));
        ButtonWidget next = ButtonWidget.Cast(m_Root.FindAnyWidget("tfm_hint_next"));

        if (showArrows)
        {
            if (prev) m_Prev = new TFM_MenuButton(prev, TFM_BtnStyle.SECONDARY);
            if (next) m_Next = new TFM_MenuButton(next, TFM_BtnStyle.SECONDARY);
        }
        else
        {
    ...
    protected void Apply(int index)
    {
        ...
        TFM_Hint hint = m_Hints.Get(index);
